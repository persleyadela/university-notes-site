...
![[]{#_Ref329585703 .anchor}图 5-1 阿尔金断裂台站分布](./media/image1.jpeg){alt="ATF" width="4.6090277777777775in" height="3.392361111111111in"}
...
反演采用均一半空间作为初始模型，初始电阻率设置为 100 Ω·m（见[表 5-1](#_Ref353821397)），即 log10ρ=2.0。最大迭代次数设置为 30 次，目标 RMS 拟合差设为 1.2。初始模型的 RMS misfit 约为 6.49，随着迭代进行，观测数据与模型响应之间的失配逐渐降低。最终反演进行至第 14 次迭代，RMS misfit 降至 1.306，模型粗糙度为 194.48，步长为 2.38×10⁻²。此时 RMS 已非常接近预设目标值 1.2，相比初始模型显著降低，说明 TM 数据与二维模型之间获得了较好的拟合。程序同时提示拟合差可能已经接近当前参数设置下能够达到的最低水平，因此最终采用第 14 次迭代模型作为 TM 模式的反演结果。
  ...
  : []{#_Ref353821397 .anchor}表 5-1 TM 模式二维反演主要参数及结果
...
2.  过程图件分析
    ...
    ![](./media/image2.png){width="3.7597222222222224in" height="3.667361111111111in"}

    []{#_Ref330173948 .anchor}图 5-2 QLM22c测点视电阻率与阻抗相位曲线
    ...
    ![](./media/image3.png){width="4.778472222222222in" height="2.467361111111111in"}

    []{#_Ref330476474 .anchor}图 5-3阿尔金断裂MT测线视电阻率与阻抗相位拟断面
    ...
    ![](./media/image4.png){width="4.436111111111111in" height="2.7895833333333333in"}

    []{#_Ref331669771 .anchor}图 5-4 MT测线相位张量与感应箭头拟断面
...
![[]{#_Ref332291630 .anchor}图 5-5 MT测点原始位置与二维剖面投影位置](./media/image5.png){alt="IMG_256" width="1.836111111111111in" height="4.309722222222222in"}
...
![[]{#_Ref332577349 .anchor}图 5-6阿尔金断裂MT剖面Occam2D反演网格](./media/image6.png){alt="IMG_256" width="4.096527777777778in" height="5.0256944444444445in"}
...
![[]{#_Ref332795840 .anchor}图 5-7 ATF剔除 QLM17a 测点前后二维 MT 反演电阻率模型对比](./media/image7.png){width="4.852083333333334in" height="4.354861111111111in"}
...
图中（a）全部测点参与反演的 TE+TM 联合反演结果；（b）剔除 QLM17a 测点后的 TE+TM 联合反演结果。两组模型采用统一电阻率色标及横纵坐标，用于比较 QLM17a 测点对二维反演电性结构及数据拟合的影响。
  ...
  : []{#_Ref354644940 .anchor}表 5-2 剔除异常测点前后的反演参数对比
...
![[]{#_Ref333938716 .anchor}图 5-8 ATF不同反演模式下二维 MT 电阻率模型对比](./media/image8.png){width="5.561111111111111in" height="7.5472222222222225in"}

a.  为 TE 模式反演结果，（b）为 TM 模式反演结果，（c）为 TE+TM 联合反演结果，（d）为 TE+TM+TP/TIPPER 联合反演结果。各子图采用统一电阻率色标及横纵坐标，以比较不同数据模式对地下电性结构成像结果的影响。
  ...
  : []{#_Ref355670167 .anchor}表 5-3 不同极化模式反演参数与拟合结果对比
...
![[]{#_Ref401620505 .anchor}图 5-9 圣安德烈斯断裂台站分布](./media/image9.jpeg){alt="SAF地理位置" width="4.683333333333334in" height="3.4472222222222224in"}
...
![[]{#_Ref334997557 .anchor}图 5-10 SAF剖面不同参数配置及测点筛选条件下的二维MT反演电阻率模型对比](./media/image10.png){width="6.219444444444444in" height="3.292361111111111in"}
...
（a）基础参数方案，全部测点参与反演；（b）New参数方案，全部测点参与反演；（c）基础参数方案，剔除 mvx030 和 mvx031 后的反演结果。各模型采用统一电阻率色标及横纵坐标，用于比较参数设置及异常测点筛选对二维电性结构反演结果的影响。
  ...
  : []{#_Ref356644973 .anchor}表 5-4 SAF 的二维 MT 不同反演方案及最终反演结果对比
...
![[]{#_Ref335552188 .anchor}图 5-11 SAF剖面 New 参数方案下不同数据模式的二维 MT 反演电阻率模型对比](./media/image11.png){width="6.207638888888889in" height="2.7555555555555555in"}
...
（a）TE+TM+TP联合反演；（b）TE-only 反演；（c）TM-only 反演；（d）TE+TM 联合反演。各模型采用统一电阻率色标以及横纵坐标，展示不同数据模式对 SAF 剖面二维电性结构反演结果的影响。
  ...
  : []{#_Ref357081955 .anchor}表 5-5 SAF 剖面 New 方案下不同反演模式结果对比
